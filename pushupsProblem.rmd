...
Position is categorical, pushups is numerical. So we use numerical - catergorical graphical - box plot
Summary - data  dataset
inference - t test compares averages (means)
...
```
This box plot shows the correlation between the position of a player and how many pushups it can do.
...
summary(players)
```
This illustration shows the summary of the players dataset.
...
```
t-test shows the results of players stats by position.
...
The t-test to compare means has a P-value of .000009421
this is less than 0.05, therefore we reject the null hypothesis and conclude that we have evidence there is a difference in the average number of pushups that can be done in 2 minutes by line an skill players.
